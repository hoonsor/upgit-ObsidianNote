...
modified: Tuesday, December 27th 2022, 10:54:56 pm
---
[[●專案管理]]
...
### 5.4. %
-  #h/red **% 則是另一個常出現的單位，它較為簡單，就只會跟從上一個的大小，而去改變。** 例如上一個的 div 為 100px 寬，身為 40% 的它，就會佔上一個 div 當中的 40%，所以也就會是 40px。
...
- 例如上一個 div 為 50%，這裡設定了寬度為 70w，我們會看到這裡預設狀況不會受到方格的限制，仍然能夠讓寬度調整為 70%，這是因為 #h/red **在 CSS 裡，有一個設定名為 overflow，預設的是 visible**：代表會見到超出方格的部分，但假若我們在第一個 div 裡加入了編碼 overflow：hidden；則會隱藏掉超出的部分。
